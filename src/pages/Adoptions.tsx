...
import { mockAdoptions } from '../data/news';
...
export default function Adoptions() {
  ...
  const filtered = mockAdoptions.filter(d => (selectedProvince === 'Toutes' || d.province === selectedProvince) && (selectedDomaine === 'Tous' || d.domaine === selectedDomaine));
    ...
    <div className="space-y-8 max-w-[1400px] mx-auto">
      <div className="flex items-center justify-between">
        <div className="flex items-center gap-3">
          ...
          <div>
            <h1 className="text-2xl font-bold text-white tracking-tight">Règlements récemment adoptés</h1>
            <p className="text-slate-400 text-sm mt-1">Nouvelles taxes votées et entrées en vigueur</p>
          </div>
        </div>
          ...
          <select
            ...
            className="px-4 py-2 bg-slate-800 border border-slate-700 text-sm font-medium text-white rounded-lg outline-none focus:ring-2 focus:ring-emerald-500"
          >
            {provinces.map(p => (
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
        {filtered.length === 0 ? (
          <p className="text-slate-500 col-span-full">Aucun règlement récemment adopté pour cette province.</p>
        ) : (
          filtered.map((item, idx) => (
            <div key={idx} className="bg-slate-800 rounded-2xl border border-slate-700 p-6 shadow-lg hover:border-emerald-500/50 transition-colors flex flex-col">
              ...
              <div className="flex items-center gap-3 mt-auto">
                ...
                <a
                  ...
                  target="_blank"
                  rel="noreferrer"
                  className="p-2 bg-slate-700 hover:bg-emerald-600 hover:text-white text-slate-300 rounded-xl transition-colors"
